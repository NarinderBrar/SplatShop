...
import { SsogResidentPageRenderPass, type ResidentGlobalChunk } from "./SsogResidentPageRenderPass";
...
type SsogGlobalSortMode = "off" | "packed" | "expanded" | "resident";
...
const getSsogGlobalSortMode = (): SsogGlobalSortMode => {
  ...
  if (value === "off" || value === "expanded" || value === "resident") {
    return value;
  }
  if (value === "packed") {
    return "packed";
...
class StreamingSsogRenderPass {
  ...
  private readonly decodedUploadPriorityBuckets: DecodedChunk[][] = Array.from(
    { length: 6 },
    () => [],
  );
  private decodedUploadQueueBytes = 0;
  ...
  private residentGlobalRuntime?: SsogResidentPageRenderPass;
  ...
  dispose(): void {
    ...
    this.decodedUploadQueueBytes = 0;
  ...
  setVizMode(mode: number): void {
    ...
    if (this.residentGlobalRuntime) {
      this.residentGlobalRuntime.setVizMode(mode);
    }
  ...
  getStats(): StreamingSsogRenderStats {
    ...
    const packedGlobalStats = this.packedGlobalRuntime?.getStats();
    const residentGlobalStats = this.residentGlobalRuntime?.getStats();
    const residentGlobalActive = residentGlobalStats?.residentGlobalActive ?? false;
    const activeStats = [
      ...(this.expandedRuntime ? [this.expandedRuntime.pass.getStats()] : []),
      ...(packedGlobalStats ? [packedGlobalStats] : []),
    ...
    const renderSplats = residentGlobalActive
      ? residentGlobalStats?.residentGlobalDrawSplats ?? 0
      : activeStats.reduce((sum, item) => sum + item.renderSplats, 0);
    const rendererModeSuffix = residentGlobalActive ? "gpu-radix" : first?.rendererMode ?? "loading";
    ...
    return {
      ...
      reverseDepthActive: activeStats.some((item) => item.reverseDepthActive),
      reverseDepthSupported: activeStats.some((item) => item.reverseDepthSupported),
      reverseDepthFallbackReason: joinFallbackReasons(activeStats.map((item) => item.reverseDepthFallbackReason)),
      reverseDepthClearValue: activeStats.some((item) => item.reverseDepthActive) ? 0 : 1,
      reverseDepthCompare: activeStats.some((item) => item.reverseDepthActive) ? "greater" : "less",
      reverseDepthNear: first?.reverseDepthNear ?? 0,
      reverseDepthFar: first?.reverseDepthFar ?? 0,
      reverseDepthFarToNearRatio: first?.reverseDepthFarToNearRatio ?? 0,
      frameTargetsMode: first?.frameTargetsMode ?? "off",
      frameTargetsSupported: activeStats.some((item) => item.frameTargetsSupported),
      frameTargetsAllocated: activeStats.some((item) => item.frameTargetsAllocated),
      frameTargetsWidth: first?.frameTargetsWidth ?? 0,
      frameTargetsHeight: first?.frameTargetsHeight ?? 0,
      frameTargetsScale: first?.frameTargetsScale ?? 1,
      frameTargetsAttachments: first?.frameTargetsAttachments ?? "",
      frameTargetsHasDepth: activeStats.some((item) => item.frameTargetsHasDepth),
      frameTargetsSamples: first?.frameTargetsSamples ?? 1,
      frameTargetsVersion: Math.max(0, ...activeStats.map((item) => item.frameTargetsVersion)),
      frameTargetsFallbackReason: joinFallbackReasons(activeStats.map((item) => item.frameTargetsFallbackReason)),
      temporalMode: first?.temporalMode ?? "off",
      temporalEnabled: activeStats.some((item) => item.temporalEnabled),
      temporalStable: activeStats.some((item) => item.temporalStable),
      temporalStableFrames: first?.temporalStableFrames ?? 0,
      temporalSampleIndex: first?.temporalSampleIndex ?? 0,
      temporalMaxSamples: first?.temporalMaxSamples ?? 0,
      temporalJitterX: first?.temporalJitterX ?? 0,
      temporalJitterY: first?.temporalJitterY ?? 0,
      temporalJitterPixelsX: first?.temporalJitterPixelsX ?? 0,
      temporalJitterPixelsY: first?.temporalJitterPixelsY ?? 0,
      temporalResetCount: Math.max(0, ...activeStats.map((item) => item.temporalResetCount)),
      temporalResetReason: first?.temporalResetReason ?? "",
      ...
      sortQueued: activeStats.some((item) => item.sortQueued),
      sortCoalesced: activeStats.reduce((sum, item) => sum + item.sortCoalesced, 0),
      sortThrottled: activeStats.reduce((sum, item) => sum + item.sortThrottled, 0),
      sortTinyReuse: activeStats.reduce((sum, item) => sum + item.sortTinyReuse, 0),
      sortMinIntervalMs: first?.sortMinIntervalMs ?? 0,
      sortDedicatedWorker: activeStats.some((item) => item.sortDedicatedWorker),
      ...
      gpuDepthKeyEnabled: residentGlobalActive || activeStats.some((item) => item.gpuDepthKeyEnabled),
      gpuDepthKeyDispatched:
        residentGlobalActive ? residentGlobalStats?.residentGlobalGpuSorted ?? false : activeStats.some((item) => item.gpuDepthKeyDispatched),
      lastGpuDepthKeyMs: residentGlobalActive
        ? residentGlobalStats?.residentGlobalGpuDepthMs ?? 0
        : activeStats.reduce((sum, item) => sum + item.lastGpuDepthKeyMs, 0),
      lastGpuDepthKeySplats: residentGlobalActive
        ? residentGlobalStats?.residentGlobalDrawSplats ?? 0
        : activeStats.reduce((sum, item) => sum + item.lastGpuDepthKeySplats, 0),
      ...
      gpuRadixSortEnabled: residentGlobalActive || activeStats.some((item) => item.gpuRadixSortEnabled),
      gpuRadixSortDispatched:
        residentGlobalActive ? residentGlobalStats?.residentGlobalGpuSorted ?? false : activeStats.some((item) => item.gpuRadixSortDispatched),
      lastGpuRadixSortMs: residentGlobalActive
        ? residentGlobalStats?.residentGlobalGpuSortMs ?? 0
        : activeStats.reduce((sum, item) => sum + item.lastGpuRadixSortMs, 0),
      lastGpuRadixSortSplats: residentGlobalActive
        ? residentGlobalStats?.residentGlobalDrawSplats ?? 0
        : activeStats.reduce((sum, item) => sum + item.lastGpuRadixSortSplats, 0),
      gpuRadixSortBits: residentGlobalActive ? 20 : first?.gpuRadixSortBits ?? 0,
      gpuRadixSortPasses: residentGlobalActive ? 5 : first?.gpuRadixSortPasses ?? 0,
      gpuSortVisibleMode: residentGlobalActive ? "radix" : first?.gpuSortVisibleMode ?? "cpu",
      gpuSortVisibleEffective: residentGlobalActive ? "radix" : first?.gpuSortVisibleEffective ?? "cpu",
      ...
      gpuUniformArenaBytes: activeStats.reduce((sum, item) => sum + item.gpuUniformArenaBytes, 0),
      gpuUniformArenaCapacityBytes: activeStats.reduce((sum, item) => sum + item.gpuUniformArenaCapacityBytes, 0),
      gpuUniformArenaAllocations: activeStats.reduce((sum, item) => sum + item.gpuUniformArenaAllocations, 0),
      gpuUniformArenaUpdates: activeStats.reduce((sum, item) => sum + item.gpuUniformArenaUpdates, 0),
      gpuUniformArenaFrameUpdates: activeStats.reduce((sum, item) => sum + item.gpuUniformArenaFrameUpdates, 0),
      gpuUniformArenaFrameUpdateBytes: activeStats.reduce((sum, item) => sum + item.gpuUniformArenaFrameUpdateBytes, 0),
      bindGroupGeneration: activeStats.reduce((sum, item) => sum + item.bindGroupGeneration, 0),
      bindGroupTrackedBuffers: activeStats.reduce((sum, item) => sum + item.bindGroupTrackedBuffers, 0),
      bindGroupContentGeneration: activeStats.reduce((sum, item) => sum + item.bindGroupContentGeneration, 0),
      bindGroupRebindAttempts: activeStats.reduce((sum, item) => sum + item.bindGroupRebindAttempts, 0),
      ...
      globalPackedCreatedBuffers: packedGlobalStats?.globalPackedCreatedBuffers ?? 0,
      globalPackedCreateStorageBufferMs: packedGlobalStats?.globalPackedCreateStorageBufferMs ?? 0,
      residentGlobalActive: residentGlobalStats?.residentGlobalActive ?? false,
      residentGlobalRebuilds: residentGlobalStats?.residentGlobalRebuilds ?? 0,
      residentGlobalMetadataBytesUploaded: residentGlobalStats?.residentGlobalMetadataBytesUploaded ?? 0,
      residentGlobalAttributeBytesReused: residentGlobalStats?.residentGlobalAttributeBytesReused ?? 0,
      residentGlobalBuildMs: residentGlobalStats?.residentGlobalBuildMs ?? 0,
      residentGlobalActiveChunks: residentGlobalStats?.residentGlobalActiveChunks ?? 0,
      residentGlobalDrawSplats: residentGlobalStats?.residentGlobalDrawSplats ?? 0,
      residentGlobalGpuDepthMs: residentGlobalStats?.residentGlobalGpuDepthMs ?? 0,
  ...
  private processDecodedUploadQueue(): number {
    ...
    const buckets = this.decodedUploadPriorityBuckets;
    for (const bucket of buckets) {
      bucket.length = 0;
    }
    for (const decoded of this.decodedUploadQueue.values()) {
      buckets[Math.min(5, this.getUploadPriority(decoded.key))].push(decoded);
    }
    for (const bucket of buckets) {
      if (bucket.length > 1) {
        bucket.sort((a, b) => a.bytes - b.bytes);
      }
    }
    ...
    for (let priority = 0; priority < buckets.length; priority++) {
      for (const decoded of buckets[priority]) {
        if (
        ...
        this.processDecodedUpload(decoded, priority, budgetState, diagnostics);
      }
    }
    for (const bucket of buckets) {
      bucket.length = 0;
  ...
  private processDecodedUpload(
    ...
    if (priority >= 5) {
      this.deleteDecodedUpload(decoded.key);
    ...
    if (this.shouldDeferDecodedUpload(decoded, priority, budgetState)) {
      diagnostics.deferredChunks++;
      diagnostics.deferredBytes += decoded.bytes;
      return;
    }
  ...
  private shouldDeferDecodedUpload(
    decoded: DecodedChunk,
    priority: number,
    budgetState: SsogUploadBudgetState,
  ): boolean {
    ...
    if (priority <= 1 || this.canSatisfyGpuPageUpload(decoded)) {
      return false;
    }
  ...
  private enqueueDecodedUpload(decoded: DecodedChunk): void {
    const previous = this.decodedUploadQueue.get(decoded.key);
    if (previous) {
      this.decodedUploadQueueBytes -= previous.bytes;
    }
    this.decodedUploadQueue.set(decoded.key, decoded);
    this.decodedUploadQueueBytes += decoded.bytes;
  }

  private deleteDecodedUpload(key: string): boolean {
    const decoded = this.decodedUploadQueue.get(key);
    if (!decoded || !this.decodedUploadQueue.delete(key)) {
      return false;
    }
    this.decodedUploadQueueBytes = Math.max(0, this.decodedUploadQueueBytes - decoded.bytes);
    return true;
  }
  ...
  private isChunkRepresentedByReadyRenderPath(key: string): boolean {
    ...
    return this.globalSortMode === "off" || this.globalSortMode === "resident";
  }
