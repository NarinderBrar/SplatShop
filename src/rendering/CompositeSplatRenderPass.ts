...
class CompositeSplatRenderPass {
  ...
  getStats(): SplatRenderStats | PackedSogRenderStats {
    ...
    return {
      ...
      frameTargetsSupported: stats.some((item) => item.frameTargetsSupported),
      frameTargetsAllocated: stats.some((item) => item.frameTargetsAllocated),
      ...
      temporalResetCount: Math.max(0, ...stats.map((item) => item.temporalResetCount)),
      ...
      sortQueued: stats.some((item) => "sortQueued" in item && item.sortQueued),
      sortCoalesced: stats.reduce((sum, item) => sum + Number(("sortCoalesced" in item ? item.sortCoalesced : 0) ?? 0), 0),
      sortThrottled: stats.reduce((sum, item) => sum + Number(("sortThrottled" in item ? item.sortThrottled : 0) ?? 0), 0),
      sortTinyReuse: stats.reduce((sum, item) => sum + Number(("sortTinyReuse" in item ? item.sortTinyReuse : 0) ?? 0), 0),
      sortMinIntervalMs: first && "sortMinIntervalMs" in first ? first.sortMinIntervalMs : 0,
      sortDedicatedWorker: stats.some((item) => "sortDedicatedWorker" in item && item.sortDedicatedWorker),
