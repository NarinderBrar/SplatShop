...
var<storage, read> chunkTable: array<vec4f>;
var<storage, read> sortedRefs: array<u32>;
...
fn physicalSplatIndex(chunk: u32, index: u32) -> u32 {
  return u32(chunkRow(chunk, 2u).x) + index;
}

fn scaleCodebookOffset(chunk: u32) -> u32 {
  return u32(chunkRow(chunk, 2u).y);
}

fn decodeCenter(chunk: u32, index: u32) -> vec3f {
  let physicalIndex = physicalSplatIndex(chunk, index);
...
fn decodeRotation(chunk: u32, index: u32) -> vec4f {
  let pixel = quatsBuffer[physicalSplatIndex(chunk, index)];
...
fn decodeScale(chunk: u32, index: u32) -> vec3f {
  let pixel = scalesBuffer[physicalSplatIndex(chunk, index)];
  let codebookOffset = scaleCodebookOffset(chunk);
  return vec3f(
...
fn main(input: VertexInputs) -> FragmentInputs {
  ...
  let chunk = chunkOrdinal(encoded);
  let index = localIndex(encoded);
  let physicalIndex = physicalSplatIndex(chunk, index);
  let splatState = splatStateBuffer[physicalIndex];
  ...
  let center = decodeCenter(chunk, index);
  ...
  if (uniforms.vizMode >= 1.0) {
    ...
    if (uniforms.vizMode == 2.0) {
      ...
      let groupId = f32(chunk * 4096u + (index / 4096u));
      let palette = vec3f(
      ...
      vertexOutputs.vColor = colorBuffer[physicalIndex];
    }
    if (isSelected) {
  ...
  let rotation = normalize(decodeRotation(chunk, index));
  let logScale = decodeScale(chunk, index);
  vertexOutputs.position = initCornerCov(center, rotation, exp(logScale), corner, centerClip);
  vertexOutputs.vCorner = corner;
  let splatColor = colorBuffer[physicalIndex];
