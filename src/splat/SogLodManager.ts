...
class SogLodManager {
  ...
  select(optionsOrBudget: SogLodSelectOptions | number, cameraPosition?: Vector3): SogLodSelection {
    const options =
      typeof optionsOrBudget === "number"
        ...
        : optionsOrBudget;
    const splatCount = this.centers.length / 3;
  ...
  private buildChunks(): SogLodChunk[] {
    const splatCount = this.centers.length / 3;
    const chunkCount = Math.ceil(splatCount / this.chunkSize);
    const chunks: SogLodChunk[] = [];
    ...
    for (let chunk = 0; chunk < chunkCount; chunk++) {
      ...
      for (let i = start; i < end; i++) {
        const offset = i * 3;
        const x = this.centers[offset + 0];
        const y = this.centers[offset + 1];
        const z = this.centers[offset + 2];
  ...
  private getChunkPriority(chunk: SogLodChunk, options: SogLodSelectOptions): number {
...
export { SogLodManager };
export type { SogLodChunk, SogLodSelection, SogLodSelectOptions };
